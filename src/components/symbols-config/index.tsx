import { type StoreSymbol, useAppStore } from "../../viewmodels/store";
...
export default function SymbolsConfig() {
  ...
  const removeSymbol = useAppStore((st) => st.removeSymbol);
    ...
    <div className="card">
      ...
      <div className="form-group">
        {Object.values(symbolsConfig).map((s) => (
          <div key={s.ticker}>
            <Accordion
              headContent={(isOpen) => (!isOpen ? <CompactSymbolClosed {...s} /> : <CompactSymbolOpen {...s} />)}
            >
              <SymbolConfig state={s} onChange={updateSymbolParams} onRemove={removeSymbol} />
            </Accordion>
          </div>
        ))}
      </div>
...
const CompactSymbolOpen = (s: StoreSymbol) => {
  return (
    <div className="ticker-preview-card">
      {/* Header Section */}
      <div className="ticker-card-header">
        <span className="ticker-badge" style={{ color: s.color }}>
...
const CompactSymbolClosed = (s: StoreSymbol) => {
  return (
    <div className="ticker-preview-card">
      {/* Header Section */}
      <div className="ticker-card-header">
      ...
      <div className="ticker-metrics-grid">
        <div className="metric-row current-price-row">
          ...
          <b className="metric-value">${Number(s.currentPrice).toLocaleString()}</b>
        </div>

        <div className="metric-row long-limit-row">
          <span className="metric-label">Stop Loss COMPRAS</span>
          <b className="metric-value long-text">${Number(s.longsStop).toLocaleString()}</b>
        </div>

        <div className="metric-row short-limit-row">
          <span className="metric-label">Stop Loss VENTAS</span>
          <b className="metric-value short-text">${Number(s.shortsStop).toLocaleString()}</b>
        </div>
      </div>
    </div>
  );
